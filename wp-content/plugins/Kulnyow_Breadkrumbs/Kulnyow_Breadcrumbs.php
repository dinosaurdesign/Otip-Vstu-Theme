<?php
/**
 * Plugin name: Redline Header
 * DEscription: Вывод хлебных крошек на страницах сайта (кроме главной) в Header
 * Version: 1.0.4
...
function dds_breadcrumbs( $sep = ' » ', $l10n = array(), $args = array() ) {
	$kb = new DDS_Breadcrumbs;
	echo $kb->get_crumbs( $sep, $l10n, $args );
}

class DDS_Breadcrumbs {
	...
	static $args = array(
		...
		// array( 'wrappatt'=>'<div class="dds_breadcrumbs">%s</div>', 'linkpatt'=>'<a href="%s">%s</a>', 'sep_after'=>'', )
	...
	function get_crumbs( $sep, $l10n, $args ) {
		...
		$loc = (object) array_merge( apply_filters( 'dds_breadcrumbs_default_loc', self::$l10n ), $l10n );
		$arg = (object) array_merge( apply_filters( 'dds_breadcrumbs_default_args', self::$args ), $args );
		...
		if ( 1 ) {
			...
			if ( ! $mark ) {
				$mark = array(
					'wrappatt'  => '<div class="dds_breadcrumbs">%s</div>',
            ...
            elseif ( $mark === 'rdf.data-vocabulary.org' ) {
				$mark = array(
					'wrappatt'  => '<div class="dds_breadcrumbs" prefix="v: http://rdf.data-vocabulary.org/#">%s</div>',
            ...
            elseif ( $mark === 'schema.org' ) {
				$mark = array(
					'wrappatt'  => '<div class="dds_breadcrumbs" itemscope itemtype="http://schema.org/BreadcrumbList">%s</div>',
        ...
        elseif ( is_singular() && $ptype->hierarchical ) {
			...
			if ( $term && isset( $term->term_id ) ) {
				$term = apply_filters( 'dds_breadcrumbs_term', $term );
		...
		$home_after = apply_filters( 'dds_breadcrumbs_home_after', '', $linkpatt, $sep, $ptype );
		...
		$out = apply_filters( 'dds_breadcrumbs_pre_out', $out, $sep, $loc, $arg );

		$out = sprintf( $wrappatt, $before_out . $out );

		return apply_filters( 'dds_breadcrumbs', $out, $sep, $loc, $arg );
	}
